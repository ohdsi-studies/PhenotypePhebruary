...
WHERE YEAR(C.condition_start_date) - P.year_of_birth >= 20
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,5,P.START_DATE) AND A.START_DATE <= DATEADD(day,90,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
